...
const HeroSection = ({ isAdmin, ko, onTour }: { isAdmin: boolean; ko: boolean; onTour: () => void }) => {
  ...
  return (
    <section className="relative bg-background-light dark:bg-background-dark">
      ...
      {/* mt 는 mt-4 대신 임의값 — common.css 의 레거시 유틸(.mt-4 { margin-top: 2rem })이
          Tailwind 의 .mt-4(1rem)를 덮어써 헤더 아래 여백이 두 배(32px)로 벌어진다.
          /about·/greeting 히어로 카드와 같은 16px 을 확실히 쓰기 위해 임의값으로 고정. */}
      <div className="relative overflow-hidden mx-5 mt-[1rem] sm:mt-5 rounded-[24px] ring-1 ring-black/[0.06] dark:ring-white/10 shadow-[0_16px_40px_-24px_rgba(0,0,0,0.45)] dark:shadow-[0_16px_40px_-24px_rgba(0,0,0,0.8)] lg:mx-0 lg:mt-0 lg:rounded-none lg:ring-0 lg:shadow-none">
        {heroBackgroundUrl ? (
          <img
