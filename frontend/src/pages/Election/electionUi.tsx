...
/** 투표율 막대 — 득표를 가려도 이것만은 늘 보인다 */
export const TurnoutBar = ({ voted, total }: { voted: number; total: number }) => {
  const percent = turnoutPercent(voted, total)
  return (
    <div>
      <div className="flex items-baseline justify-between gap-2">
        <span className="text-[12px] font-semibold text-ink-muted">투표율</span>
        <span className="text-[12.5px] font-bold text-ink-strong tabular-nums">
          {voted}
          <span className="text-ink-muted font-semibold"> / {total}명 · {percent}%</span>
        </span>
      </div>
      <div className="mt-1.5 h-1.5 rounded-full bg-gray-100 dark:bg-white/[0.08] overflow-hidden">
        <div
