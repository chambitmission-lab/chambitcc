 ...
 *
 * 소요 시간은 경로 API 없이 직선거리로 어림한 값이라, 화면에서도 어림값임을
 * 분명히 말하고 정확한 경로는 지도 앱에 넘긴다. 지어낸 ETA 를 단정하지 않는다.
 */
const LeaveNowCard = ({
  church,
  nextServiceName,
  ...
  const verdict: Verdict | null =
    ...
    nextServiceInMin > ACTIONABLE_WINDOW_MIN
      ? null
  ...
  return (
    <section className="visit-now">
      ...
      {!church && !nextServiceName && (
        <p className="visit-now-note">{ko ? '준비 중입니다.' : 'Coming soon.'}</p>
      )}
    </section>
  )
}
