  ...
  return (
    <PastorShell>
      <div className="px-4 pt-3">
        <Link to="/pastor/members" className="inline-flex items-center gap-1 text-[12.5px] font-semibold text-gray-600 dark:text-white/65 hover:text-brand">
      ...
      {isPending && !data ? (
        ...
        <p className="px-4 py-16 text-center text-[13px] text-gray-600 dark:text-white/60">성도 정보를 불러오지 못했습니다</p>
      ) : (
        <div className="contents lg:grid lg:grid-cols-[minmax(0,1fr)_380px] lg:items-start">
          <div className="contents lg:block lg:min-w-0">
            ...
            <VisitsCard
              ...
              onPlan={() => setVisitTarget({ initialStatus: 'planned' })}
              onEdit={v => setVisitTarget({ visit: v })}
              onComplete={v => setVisitTarget({ visit: v, completing: true })}
            />
          </div>
  ...
  return (
    <div className="px-4 pt-3">
      <div className="relative overflow-hidden rounded-2xl bg-white/80 dark:bg-card-dark border border-gray-200/70 dark:border-white/[0.08] shadow-sm p-5">
        ...
        {!p ? (
          <p className="mt-4 py-4 text-center text-[12.5px] text-gray-500 dark:text-white/55 rounded-xl border border-dashed border-gray-200 dark:border-white/[0.1]">
          ...
          <>
            <dl className="mt-4 grid grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-3">
              <Info label="생일">
                ...
                {p.birthday && <span className="text-gray-500"> · {p.birthday.slice(0, 4)}년생</span>}
              </Info>
              <Info label="연락처">
                {p.phone ? (
            ...
            {p.pastoral_note && (
              <div className="mt-4 rounded-xl bg-[var(--brand-soft)] px-3.5 py-3">
                <p className="text-[12px] font-bold text-brand">함께 기억할 것</p>
                <p className="mt-1 text-[13px] text-ink-strong whitespace-pre-wrap leading-relaxed">{p.pastoral_note}</p>
              </div>
            )}
            {p.updated_at && (
              <p className="mt-3 text-[12px] text-gray-500 dark:text-white/50">
...
const Info = ({ label, children, wide }: { label: string; children: ReactNode; wide?: boolean }) => (
  <div className={wide ? 'col-span-2 lg:col-span-3' : ''}>
    <dt className="text-[12px] font-semibold text-gray-500 dark:text-white/55">{label}</dt>
...
const VisitsCard = ({
  ...
  onComplete: (v: PastoralVisit) => void
}) => {
  // 예약(아직 안 감)과 다녀온 기록을 나눠 보여 준다 — 예약은 위, 기록은 타임라인
  const plans = data.visits
    .filter(v => v.status === 'planned')
    .sort((a, b) => (a.visit_date + (a.visit_time ?? '')).localeCompare(b.visit_date + (b.visit_time ?? '')))
  const done = data.visits.filter(v => v.status !== 'planned')
  return (
  <SectionCard
    title={`심방 기록 ${done.length || ''}`.trim()}
    action={
  ...
  >
    {plans.length > 0 && (
      <ul className="space-y-1.5">
        {plans.map(v => {
          const due = v.visit_date <= todayIso()
          return (
            <li
              key={v.id}
              className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl border ${
                due && v.visit_date < todayIso()
                  ? 'border-[var(--amber-soft-strong)] bg-[var(--amber-soft)]'
                  : 'border-[var(--brand-soft-strong)] bg-[var(--brand-soft)]'
              }`}
            >
              <span className="material-icons-outlined text-[20px] text-brand">event_available</span>
              <span className="flex-1 min-w-0">
                <span className="block text-[13px] font-bold text-ink-strong">
                  {formatDay(v.visit_date)} {v.visit_time ?? ''} · {VISIT_KIND_LABEL[v.kind]} 예약
                </span>
                <span className="block text-[12px] text-gray-600 dark:text-white/60 truncate">
                  {v.is_mine ? '나' : v.pastor_name}
                  {v.is_mine && v.summary ? ` · ${v.summary}` : ''}
                </span>
              </span>
              {v.is_mine && (
                <span className="shrink-0 flex gap-1">
                  {due && (
                    <button type="button" onClick={() => onComplete(v)} className="px-2.5 py-1.5 rounded-full bg-brand text-white text-[12px] font-bold">
                      다녀왔어요
                    </button>
                  )}
                  <button type="button" onClick={() => onEdit(v)} className="text-[12px] font-semibold text-gray-600 hover:text-brand px-1">
                    고치기
                  </button>
                </span>
              )}
          ...
          <li key={v.id} className="relative flex gap-3">
            <span
              className={`relative z-10 shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
                v.is_mine ? 'bg-brand text-white' : 'bg-gray-100 dark:bg-white/[0.08] text-gray-600 dark:text-white/65'
              }`}
            >
            ...
            <div className="flex-1 min-w-0 pt-0.5">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {v.is_mine && (
                  <button
                    type="button"
                    onClick={() => onEdit(v)}
                    className="ml-auto text-[12px] font-semibold text-gray-500 hover:text-brand"
                  >
                    고치기
                  </button>
                )}
              ...
              {v.is_mine ? (
                  ...
                  {v.follow_up && (
                    <p
                      className={`mt-1.5 inline-flex items-center gap-1 text-[12px] font-semibold px-2 py-1 rounded-lg ${
                        v.follow_up_done
                          ? 'bg-gray-100 dark:bg-white/[0.05] text-gray-500 line-through'
                ...
                <p className="mt-1 text-[12px] text-gray-500 dark:text-white/50">내용은 작성한 교역자만 볼 수 있습니다</p>
              )}
            </div>
          </li>
        ))}
...
const BRIEF_TONE: Record<Briefing['points'][number]['tone'], string> = {
  info: 'text-gray-500 dark:text-white/55',
...
const BriefingCard = ({ memberId, enabled }: { memberId: number; enabled: boolean }) => {
  ...
  return (
    <SectionCard title="심방 전 브리핑">
      ...
      <p className="text-[12px] text-gray-500 dark:text-white/50">명부·심방·기도·활동 기록을 규칙으로 엮은 요약입니다.</p>
      </>
      )}
    </SectionCard>
  )
  ...
  <SectionCard title="앱에서의 발걸음">
    <div className="grid grid-cols-2 gap-2">
      <div className="rounded-xl bg-gray-50 dark:bg-white/[0.03] border border-gray-100 dark:border-white/[0.05] px-3.5 py-3">
        <p className="text-[12px] font-semibold text-gray-600 dark:text-white/65">마지막 활동</p>
        <p
      ...
      <div className="rounded-xl bg-gray-50 dark:bg-white/[0.03] border border-gray-100 dark:border-white/[0.05] px-3.5 py-3">
        <p className="text-[12px] font-semibold text-gray-600 dark:text-white/65">최근 1년 기록</p>
        <p className="mt-0.5 text-[18px] font-bold tracking-[-0.02em]">
          ...
          <span className="text-[12px] font-semibold text-gray-500 ml-0.5">건</span>
        </p>
      </div>
    </div>
          ...
          <li key={p.id}>
            ...
            >
              <span className="flex items-center gap-1.5">
                <span className="text-[12px] text-gray-500 dark:text-white/55">{formatDay(p.created_at, false)}</span>
                {p.is_answered ? (
                  <span className="text-[12px] font-bold text-[var(--amber)]">응답됨</span>
                ) : p.pastor_replied ? (
                  <span className="text-[12px] font-bold text-brand">답함</span>
                ) : (
                  <span className="text-[12px] font-bold text-gray-600 dark:text-white/65">답 기다림</span>
                )}
              </span>
