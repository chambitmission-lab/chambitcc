...
const ThanksComposer = ({ month }: { month: string }) => {
  ...
  return (
    <div className="mt-4 rounded-xl border border-gray-100 dark:border-white/[0.06] p-3.5 lg:p-5">
      <p className="text-[13px] lg:text-[16px] font-bold text-ink-strong">얼굴 모르는 기도자에게 고마움 전하기</p>
      <p className="mt-0.5 text-[12px] lg:text-[14.5px] text-[var(--text-muted)] lg:text-[var(--text-body)] break-keep">
      ...
      <div className="mt-2.5 flex flex-wrap gap-1.5">
        {THANKS_PRESETS.map((preset) => (
          <button
            ...
            className={`px-3 h-8 lg:px-4 lg:h-11 rounded-full text-[12.5px] lg:text-[15px] font-semibold border transition-colors ${
              text === preset
      ...
      <div className="mt-2 flex gap-2">
        <input
          ...
          className="flex-1 min-w-0 h-10 lg:h-12 rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/[0.04] px-3 lg:px-4 text-[13.5px] lg:text-[16px] text-ink-strong placeholder:text-gray-400 dark:placeholder:text-white/35 focus:outline-none focus:border-[var(--brand)]"
        />
        <button
          type="button"
          disabled={!body || send.isPending}
          onClick={() => send.mutate(body)}
          className="shrink-0 h-10 lg:h-12 px-4 lg:px-6 rounded-xl bg-[var(--brand)] text-[var(--on-brand)] text-[13.5px] lg:text-[16px] font-bold disabled:opacity-50"
        >
          전하기
        </button>
      </div>
...
export const RecapCard = ({ recap }: { recap: IntercessionRecap }) => {
  ...
  return (
    <section className="relative overflow-hidden mx-4 mt-5 rounded-[26px] px-5 pt-6 pb-5 lg:px-8 lg:pt-8 lg:pb-7 bg-white dark:bg-card-dark ring-1 ring-[var(--brand-soft-strong)] shadow-[0_10px_30px_-18px_rgba(49,130,246,0.5)]">
      ...
      {recap.was_receiver ? (
        <>
          <h2 className="mt-2 text-[20px] lg:text-[26px] font-extrabold tracking-[-0.02em] leading-[1.35] text-ink-strong whitespace-pre-line break-keep">
          ...
          {recap.letters_received > 0 ? (
            <p className="mt-3 text-center text-[12.5px] lg:text-[15px] font-semibold text-brand">
              아래에 {recap.letters_received > 1 ? `편지 ${recap.letters_received}통이` : '편지 한 통이'} 도착해 있어요
            </p>
          ) : null}

          {recap.thanks_sent ? (
            <p className="mt-4 rounded-xl bg-[var(--brand-soft)] px-3.5 py-3 lg:px-4 lg:py-3.5 text-[13px] lg:text-[16px] leading-relaxed text-ink-strong break-keep">
        ...
        <h2 className="mt-2 text-[20px] lg:text-[26px] font-extrabold tracking-[-0.02em] leading-[1.35] text-ink-strong break-keep">
          {month}에도 함께해 주셔서 고마워요
        </h2>
      )}

      {recap.was_giver ? (
        <div className="mt-4 rounded-xl bg-gray-50 dark:bg-white/[0.03] px-3.5 py-3 lg:px-4 lg:py-3.5">
          <p className="text-[13px] lg:text-[16px] text-[var(--text-body)] break-keep">
          ...
          {recap.thanks_received.length > 0 ? (
            <ul className="mt-2 space-y-1.5">
              {recap.thanks_received.map((t, i) => (
                <li key={i} className="text-[13px] lg:text-[16px] leading-relaxed text-ink-strong break-keep">
                  기도한 분이 보낸 고마움 · “{t}”
                </li>
              ))}
            </ul>
      ...
      <p className="mt-4 text-center text-[12px] lg:text-[15px] text-[var(--text-muted)] lg:text-[var(--text-body)] break-keep">
        {month} 우리 교회는 서로를 위해{' '}
      ...
      {recap.was_receiver ? (
        <button
          type="button"
          onClick={sealCapsule}
          className="mt-4 w-full rounded-xl border border-gray-200 dark:border-white/10 px-3.5 py-3 lg:px-4 lg:py-4 text-left hover:border-[var(--brand-soft-strong)] transition-colors"
        >
