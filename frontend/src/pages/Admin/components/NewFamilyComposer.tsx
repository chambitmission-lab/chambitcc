...
const NewFamilyComposer = ({ post, onClose, onSuccess }: NewFamilyComposerProps) => {
  ...
  // PC에선 좌(정보 입력) / 우(넓은 사진 영역) 2단으로 펼친다
  return (
    <AdminComposerShell
      title={isEdit ? '새가족 수정' : '새가족 등록'}
      onClose={onClose}
      as="form"
      onSubmit={handleSubmit}
      gridCols="lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]"
      footer={
        <ComposerFooter
          onClose={onClose}
          canSubmit={canSubmit}
          submitting={submitting}
          submitLabel={isEdit ? '수정 저장' : '새가족 등록'}
          submittingLabel={isEdit && !photosChanged ? '저장 중...' : '업로드 중...'}
          cancelDisabled
        />
      }
      columns={[
        ...
        <>
            ...
            >
              <input
                ...
                className="mt-0.5 w-4 h-4 shrink-0 rounded border-gray-300 dark:border-gray-600 accent-[var(--brand)]"
              />
