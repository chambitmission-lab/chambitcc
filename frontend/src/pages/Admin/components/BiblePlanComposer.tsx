...
const BiblePlanComposer = ({ editingPlan, onClose, onSuccess }: Props) => {
  ...
  // PC에선 좌(기본 정보) / 우(진행 방식·일정) 2단으로 펼친다
  return (
    <AdminComposerShell
      title={editingPlan ? '읽기 플랜 수정' : '새 읽기 플랜'}
      onClose={onClose}
      as="form"
      onSubmit={handleSubmit}
      gridCols="lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]"
      footer={
        <ComposerFooter
          onClose={onClose}
          canSubmit={canSubmit}
          submitting={submitting}
          submitLabel={editingPlan ? '수정 저장' : '등록하기'}
          plain
        />
      }
      columns={[
