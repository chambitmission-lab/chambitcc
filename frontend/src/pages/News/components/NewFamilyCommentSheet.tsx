...
const NewFamilyCommentSheet = ({ post, onClose }: NewFamilyCommentSheetProps) => {
  ...
  const { createComment, isCreating } = useCreateNewFamilyComment(post.id, toastFeedback<{ message?: string }>({ success: (response) => response.message, error: t('newsCommentCreateFailed') }))
  const { updateComment, isUpdating } = useUpdateNewFamilyComment(post.id, toastFeedback<{ message?: string }>({ success: (response) => response.message, error: t('newsCommentUpdateFailed') }))
  const { deleteComment } = useDeleteNewFamilyComment(post.id, toastFeedback<{ message?: string }>({ success: (response) => response.message, error: t('newsCommentDeleteFailed') }))
          ...
          <div className="flex items-center justify-between px-5 py-3 border-b border-black/[0.04] dark:border-white/[0.06]">
            <div className="min-w-0">
              <p className="text-[16px] font-bold text-ink-strong tracking-[-0.015em]">
                {t('newsNfCommentTitle')}
              </p>
              <p className="text-[12px] text-gray-500 dark:text-white/50 truncate">
                {t('newsNfCommentSubtitle').replace('{name}', post.member_name)}
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="flex items-end gap-2.5">
            ...
            <div className="flex-1 min-w-0 flex items-end gap-1.5 rounded-2xl border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark px-2 py-1.5 focus-within:border-purple-400 dark:focus-within:border-purple-400/60 transition-colors">
              ...
              <textarea
                ...
                placeholder={t('newsNfCommentPlaceholder')}
                rows={1}
                disabled={isCreating}
