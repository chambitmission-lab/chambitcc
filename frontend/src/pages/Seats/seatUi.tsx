...
export const TicketCard = ({
  ...
  return (
    <section className="relative rounded-3xl overflow-hidden shadow-[0_14px_36px_-16px_var(--brand-glow)] border border-[var(--brand-soft-strong)]">
      <div className="relative bg-brand text-white px-5 pt-4 pb-5">
        <div className="flex items-center justify-between">
          <span className="text-[10.5px] font-bold tracking-[0.28em] text-white/75">MY TICKET</span>
          <span className="text-[11px] font-bold text-white/85">{sorted.length}석</span>
        </div>
        <p className="mt-2 text-[18px] font-extrabold tracking-[-0.02em] leading-snug break-keep">
          {event.title}
        </p>
      ...
      <div className="bg-white dark:bg-card-dark px-5 pt-4 pb-4">
        <div className="flex items-end justify-between gap-3">
          <p className="text-[11px] font-bold text-gray-400 dark:text-white/40 tracking-[0.08em]">좌석</p>
          {name ? <p className="text-[12px] font-semibold text-ink-muted">{name} 님</p> : null}
        </div>
        <div className="mt-1.5 flex flex-wrap gap-1.5">
          {sorted.map((s) => (
            <span
              key={s}
              className="inline-flex items-center h-9 px-3 rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:border-emerald-500/25 text-[16px] font-extrabold tabular-nums tracking-[-0.01em]"
            >
              {s}
            </span>
          ))}
        </div>
        <p className="mt-3 text-[11.5px] text-ink-muted">입장할 때 이 화면을 보여주세요.</p>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-2">
            ...
            <span className="shrink-0 mt-0.5 flex items-center gap-1">
              {dday ? (
                <span className="inline-flex text-[10.5px] font-extrabold px-2 py-0.5 rounded-full bg-gray-900 text-white dark:bg-white dark:text-gray-900 tabular-nums leading-[1.5]">
                  {dday}
                </span>
              ) : null}
              <span className={`inline-flex text-[10.5px] font-bold px-2 py-0.5 rounded-full border leading-[1.5] ${
                soldOut
          ...
          {hasMine ? (
            <p className="mt-2 inline-flex items-center gap-1.5 text-[12.5px] font-bold text-emerald-600 dark:text-emerald-300">
            ...
            <div className="mt-2.5">
              ...
              <p className="mt-1 text-[11.5px] text-gray-400 dark:text-white/45 tabular-nums">
                남은 좌석 {event.available_count} / {event.total_seats}
              </p>
            </div>
          ) : null}

          <span className={`mt-2 inline-flex items-center gap-0.5 text-[13px] font-bold ${primary || hasMine ? 'text-brand' : 'text-gray-400 dark:text-white/45'}`}>
            {action}
