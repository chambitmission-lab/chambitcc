...
const SeatEventList = () => {
  ...
  return (
    <SurveyShell onBack={() => navigate('/')} title="좌석 예약" rail={rail}>
      <section className="relative overflow-hidden mx-4 mt-5 px-6 py-7 rounded-[26px] bg-[linear-gradient(120deg,#d4eafc_0%,#dff1ff_58%,#ebf5ff_125%)] ring-1 ring-[rgba(49,130,246,0.15)] shadow-[0_10px_30px_-14px_rgba(49,130,246,0.45)] dark:bg-[linear-gradient(120deg,#071222_0%,#0a1a35_58%,#0b1730_125%)] dark:ring-white/[0.08] dark:shadow-[0_10px_34px_-12px_rgba(0,0,0,0.6)]">
        {/* 객석 줄 장식 — 오른쪽 아래로 흐르는 의자 점 */}
        <div className="absolute right-4 bottom-4 grid grid-cols-5 gap-1.5 opacity-70" aria-hidden>
          {Array.from({ length: 15 }, (_, i) => (
            <span
              key={i}
              className={`w-3.5 h-3.5 rounded-[32%_32%_22%_22%] ${
                i === 7 || i === 8 ? 'bg-brand' : 'bg-white/80 dark:bg-white/15'
              }`}
            />
          ))}
        </div>
        <div className="relative z-10">
          ...
          <h2 className="text-[26px] font-extrabold tracking-[-0.02em] leading-[1.25] mt-3 whitespace-pre-line text-[#152648] dark:text-white">
            {mineCount > 0
              ? `예약한 행사\n${mineCount}개가 기다려요`
              : openCount > 0
                ? `원하는 자리를\n직접 골라보세요`
          ...
          <p className="text-[13px] font-light leading-[1.7] text-[#41527a] dark:text-white/80 mt-3 max-w-[13rem] break-keep">
