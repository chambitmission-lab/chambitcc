...
const GlossarySheet = ({ entry, onClose }: { entry: GlossaryEntry; onClose: () => void }) => {
  ...
  return createPortal(
    ...
    >
      <div
        className="relative w-full sm:max-w-sm lg:max-w-md sheet-rise bg-background-light dark:bg-card-dark rounded-t-3xl sm:rounded-3xl overflow-hidden border border-black/[0.04] dark:border-white/[0.08] shadow-[0_-12px_40px_rgba(0,0,0,0.5)] sm:shadow-[0_12px_40px_rgba(0,0,0,0.6)]"
      ...
      >
        ...
        <div className="relative z-10 px-5 pt-6 pb-5 lg:px-6 lg:pt-7 lg:pb-6">
          ...
          <p className="mt-4 text-[14.5px] lg:text-[length:calc(17px*var(--text-mul,1))] leading-relaxed text-ink break-keep">{entry.desc}</p>
          ...
          {firstRef ? (
            ...
            <p className="mt-3 flex items-center gap-1.5 text-[12px] lg:text-[length:calc(14px*var(--text-mul,1))] text-ink-muted lg:text-ink">
