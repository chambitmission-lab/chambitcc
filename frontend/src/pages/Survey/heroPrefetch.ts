// /survey 히어로 삽화 선요청.
//
// 삽화는 survey-hero.css 의 background-image 로 깔린다. CSS 배경은 두 겹으로 늦게 발견된다 —
...
//
// 또 브라우저는 현재 매칭되는 한 장만 받는다 (.survey-hero-art / .dark .survey-hero-art).
// 라이트로 보던 사용자가 다크로 토글하면 그 순간 반대 테마 파일을 맨땅에서 받기 시작해
// 히어로가 그라데이션만 남는다 — 그래서 현재 테마를 먼저, 반대 테마를 유휴 시간에 데운다.
//
// 호출 지점: routePreload 의 routeDataPrefetchers['/survey'] — 헤더 메뉴 호버·유휴 프리로드로
// 청크를 받는 그 순간 삽화도 같이 데운다. 그 길을 거치지 않고 바로 들어온 경우는
// SurveyList 진입이 받아 페이드인한다.

const HERO_LIGHT = '/images/survey/hero-light.webp'
const HERO_DARK = '/images/survey/hero-dark.webp'
...
export const isSurveyHeroWarm = (): boolean => settled.has(currentHero())

/** 현재 테마 삽화를 받고, 이어서 유휴 시간에 반대 테마까지 받아 둔다(토글 지연 제거) */
export const warmSurveyHero = (): Promise<void> => {
