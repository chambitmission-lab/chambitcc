    ...
    <div className="bg-[var(--app-canvas)] dark:bg-background-dark min-h-screen page-stage">
      ...
      <div className="max-w-md mx-auto bg-background-light dark:bg-background-dark min-h-screen lg:max-w-none lg:mx-0 lg:flex-1 lg:min-w-0 lg:rounded-3xl lg:border lg:border-border-light dark:lg:border-border-dark lg:overflow-hidden lg:min-h-0">
        <div className="history-page">
          {/* 새 화면 시범 운영 안내 — 의견 수렴이 끝나면 이 배너와 /history/new 중 하나를 정리한다 */}
          <button
            type="button"
            className="history-lab-banner"
            onClick={() => navigate('/history/new')}
            onPointerEnter={() => void preloadRoute('/history/new')}
          >
            <span className="history-lab-banner-badge">{ko ? '시범' : 'Beta'}</span>
            <span className="history-lab-banner-text">
              {ko ? '새로운 발자취 화면을 미리 만나 보세요' : 'Try the new History page'}
            </span>
            <span className="history-lab-banner-go" aria-hidden="true">
              {ko ? '미리 보기 →' : 'Preview →'}
            </span>
          </button>
