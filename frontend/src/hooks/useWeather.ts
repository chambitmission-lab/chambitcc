...
const WEATHER_URL =
  ...
  `&current=weather_code,is_day,temperature_2m&timezone=Asia%2FSeoul`
...
const fetchCurrentWeather = async (): Promise<CurrentWeather | null> => {
  ...
  return {
    emoji: weatherCodeToEmoji(code, (data.current?.is_day ?? 1) === 1),
    temperature: Math.round(temperature),
  }
}

/* 현재 날씨(이모지+기온). 로딩 중이거나 실패하면 null — 호출부는
