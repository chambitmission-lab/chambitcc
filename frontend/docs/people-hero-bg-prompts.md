...
## 적용 상태 (2026-09-10) — 1안 채택

- **1안(단체사진 대참사)** 라이트/다크 두 장 적용 완료.
  `src/assets/people/hero-light.webp`(30KB) · `hero-dark.webp`(27KB), 원본 1584×672(≈2.36:1).
  `public/` 이 아니라 `src/assets/` 에 둔 이유는 서비스워커 stale-while-revalidate —
  해시 URL이 붙어야 다시 구웠을 때 바로 반영된다.

### Gemini ✦ 워터마크 제거 (두 장 모두 같은 자리)
...
- ✅ **방향성 복제**: 그 자리 원본이 평평한 면들뿐이라는 걸 이용.
  `y < 547`(좌석판 윗선 위)는 **바로 위 픽셀을 아래로 복제** → 배경 면과 세로 기둥이 그대로 내려온다.
  `y >= 547`은 **바로 오른쪽 픽셀을 왼쪽으로 복제** → 좌석판·가로 살이 이어진다.
  보간(gradient)이 아니라 복제라서 평면이 평면으로 남는다. 경계 1px 깃털링 + 원본 결의 그레인.
  스크립트는 스크래치패드에 있었고, 다시 필요하면 위 규칙만 그대로 옮기면 된다.

### 레이아웃 (`people.css` 히어로 블록)

- 모바일: `background-size: 175% auto; center bottom` — **제목 줄 수와 무관하게 양들 띠 높이가 일정**(≈120px).
  `cover` 로 두면 히어로가 길어질수록 양이 커져서 글자와 부딪힌다.
- PC(≥1024): `cover` — 가로가 길어 폭 기준으로 깔면 왼쪽 화분·스툴, 오른쪽 의자·깃발까지 다 들어온다.
- 아래 여백: 모바일 `8.75rem` / PC `12.5rem`.
  **관리자에게만 보이는 '인물 관리' 버튼**이 한 줄 더 붙으므로
  `.ppl-hero:has(.ppl-hero-admin)` 로 그때만 `10.5rem` / `14.5rem` 로 늘린다
  (버튼이 점프하는 새끼양과 겹치던 문제). JSX 쪽에 `ppl-hero-admin` 클래스가 짝으로 필요하다.
- `.ppl-hero::before` 의 **브랜드 blur 원은 삭제**하고 그 자리를 **카드색 세로 페이드**로 바꿨다
  (위 24%는 불투명 → 74%에서 소멸, 맨 아래 6%는 바닥선이 카드색으로 사라지게).
  카드색은 Tailwind `background-light #f1f3f6` / `background-dark #131313` 이라 CSS에 직접 박아 뒀다 —
  이 두 값이 바뀌면 페이드도 같이 고쳐야 한다.
