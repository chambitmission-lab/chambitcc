...
- **가운데 빈 구간 연출 (PC 전용, `.notice-fly`)**: 공지 종이 한 장이 배너 36% 지점에서
  팔랑거리며 날아가 오른쪽 게시판에 붙고, 그 순간 브랜드색 링이 퍼진다(11초 주기, 비행은 3초).
  - 바깥 span 을 **배너와 같은 폭**으로 깔아 `translateX` 의 %가 곧 배너 폭 비율이 되게 했다.
    translate 의 %는 부모가 아니라 **자기 크기** 기준이라, 이렇게 해야 배너가 848~1192px
    사이에서 변해도 착지점이 안 흔들린다.
  - 착지 좌표 `calc(100% - 118px)` · 링 `right 96px / top 25px` 은 **삽화 속 게시판 종이의
    실측 위치**(크롭 기준 가로 0.33~0.55, 세로 0.28~0.49 → 배너 오른쪽에서 83~110px,
    위에서 22~40px)에서 나온 값이다. **삽화를 다시 뽑거나 크롭을 바꾸면 이 두 값도 다시 계산할 것.**
