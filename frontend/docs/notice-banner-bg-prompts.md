...
- **가운데 빈 구간 연출 (`.notice-fly`, 모바일·PC 공통)**: 공지 종이 한 장이 팔랑거리며
  날아가 오른쪽 게시판에 붙고, 그 순간 브랜드색 링이 퍼진다(11초 주기, 비행은 3초).
  - 바깥 span 을 **배너와 같은 폭**으로 깔아 `translateX` 의 %가 곧 배너 폭 비율이 되게 했다.
    translate 의 %는 부모가 아니라 **자기 크기** 기준이라, 이렇게 해야 배너가 358~1192px
    사이에서 변해도 착지점이 안 흔들린다.
  - **출발·착지는 `--fly-from` / `--fly-to` 변수**로 빼서 브레이크포인트마다 다르게 준다.
    삽화 크기가 모바일 `auto 86%` / PC `auto 100%` 로 달라 게시판 종이의 좌표가 같을 수 없다:

    | | 삽화 폭 | 게시판 종이(오른쪽에서) | `--fly-from` | `--fly-to` | 링 `right` |
    |---|---|---|---|---|---|
    | 모바일 | 105px | 74~97px (중심 85) | 52% | `calc(100% - 94px)` | 79px |
    | lg+ | 125px | 83~110px (중심 96) | 36% | `calc(100% - 106px)` | 90px |

    모바일 출발점이 52%로 더 뒤인 건 앞쪽이 제목·미리보기 글줄 자리이기 때문. 링 `top`은
    25px로 공통(세로는 두 경우 모두 위에서 ≈31px).
  - 이 좌표는 **삽화 속 게시판 종이의 실측 위치**(크롭 기준 가로 0.33~0.55, 세로 0.28~0.49)에서
    역산한 값이다. **삽화를 다시 뽑거나 크롭·`background-size` 를 바꾸면 표의 값을 다시 계산할 것.**
  - 가로 이동(바깥)과 상하 흔들림·회전(안쪽 `.notice-leaf`)을 **다른 요소로 분리**해서 곱한다.
    한 요소에 transform 두 개를 넣을 수 없기 때문.
  - `prefers-reduced-motion` 에서는 `.notice-fly`·`.notice-pin` 을 `display: none`.
