...
- 원본 1024×1024 두 장에서 제미나이 워터마크(우하단 ✦)를 마름모 마스크로 inpaint(`cv2.INPAINT_TELEA`, r=5)로 지우고,
  **가장자리 복제(`BORDER_REPLICATE`)로 사방 260px을 덧대 1544×1544** 로 저장했다.
  덧댄 260px 구간에서만 알파를 0으로 떨어뜨려(smoothstep) 라이트·다크 캔버스 어느 쪽에도 이음매 없이 녹는다.
  → `public/images/capsule/sealed-light.webp`(52KB) / `sealed-dark.webp`(36KB), q82.
  **삽화 자신의 바깥 바탕색도 카드 색으로 밀어 올렸다** — 원본 바탕은 `#ebeef3`(차가운 회색)이라
  라이트 카드(`#f1f3f6`) 위에서 회색 얼룩으로 읽혔다. 바탕 픽셀만으로(코너 flood fill 마스크 +
  normalized convolution, σ=90) '바탕색 지도'를 만들고, 금고 중심에서 r=430→690 구간에 걸쳐
  `픽셀 + w × (캔버스색 − 바탕색지도)` 로 보정한다. **저주파만 옮기므로 양·텐트·그레인은 그대로**이고,
  금고 둘레의 크림빛 조명은 남은 채 바깥만 카드 색으로 스프레이처럼 흩어진다. (다크는 r=540→720)
  **여백은 원본 가장자리를 그냥 복제하면 안 된다** — 그 줄의 그레인·얼룩이 그대로 늘어나 스프레이 자국 같은 세로 줄무늬가 생긴다.
  `GaussianBlur(sigma=34)` 한 판을 복제해 깔고 원본 안쪽 40px에서 섞은 뒤(경계 소멸),
  바깥으로 갈수록 **캔버스 색**(라이트 `#f1f3f6` / 다크 `#131313`)으로 물들여 남은 회색기까지 카드 색에 녹인다.
  **여백이 88px일 때 모바일은 멀쩡했지만 PC(885px 카드)에서 정사각 경계가 그대로 보였다** —
  모바일은 그림이 화면 폭을 꽉 채워 경계가 화면 밖에 있었을 뿐이다. 260px(화면에서 105px)이면 빛 웅덩이처럼 번진다.
  **그림 자체는 한 픽셀도 자르지 않았다** — 양·의자·텐트·자는 새끼양이 전부 보여야 한다는 게 채택 조건이었다.
- 측정값(원본 1024 기준): 안쪽 원 중심 `(512, 471)`, 반지름 `277`. 260px 덧댄 뒤 중심 `(772, 731)`.
- `capsule.css` `.capsule-waiting::before` 가 **621×621 고정 px** 로 깐다(`top: -114px`, `left: 50%`, `translateX(-50%)`).
  폭이 컨테이너가 아니라 **다이얼 208px에 묶인 고정값**인 이유: 화면 폭을 따라 늘리면 금고 문 구멍과 다이얼 링이 어긋난다.
  621px일 때 그림 본체는 412px, 안쪽 원 반지름 ≈ 111px, 다이얼 눈금 반지름 ≈ 101px — 링이 눈금 바로 바깥에 앉는다.
...
- 모바일은 `.capsule-waiting { overflow: hidden }`(가로 스크롤 방지) — 페이드 여백이 카드 밖으로 잘려 그림이 풀블리드로 보인다.
  **PC(≥1024px)에서는 `overflow: visible`** 로 되돌린다. 560px 컨테이너에 가두면 페이드 도중에 잘려 경계가 다시 생긴다.
  가로 스크롤은 부모 카드(`lg:overflow-hidden`)가 막는다.
  360px 이하 폭에서는 텐트 오른쪽 끝이 몇 px 잘린다 — 다이얼 정렬을 지키려면 감수해야 하는 부분.
- **PC에서는 조명만 카드 폭까지 넓힌다**: `@media (min-width: 1024px)` 에서 `::before` 를 `width: 1040px` 로 키우고
  배경을 두 겹으로 깐다 — `var(--vault-art)`(621px 고정, 그대로) + `var(--vault-glow)`(라디얼 조명).
  885px 카드 한가운데 621px 그림만 있으면 좌우가 빈 접시처럼 보이는데, 그림을 키워 메우면 다이얼과 어긋난다.
  그래서 **그림이 아니라 빛만** 넓힌다. 조명색은 그림에서 뽑았다 — 라이트 크림 `#fdf9ed`, 다크는 등불 주변의 따뜻한 어둠.
  두 값 모두 `.capsule-waiting` / `.dark .capsule-waiting` 의 CSS 변수라 테마 교체는 변수만 바뀐다.
