...
2. 제미나이 출력은 **1792×592, 3:1** 로 받는다. 자르지 말고 그대로 후처리로 넘긴다.
3. `~/Downloads/1.png`(라이트) `2.png`(다크)로 두고

   ```
   python docs/plan-hero-process.py ~/Downloads classes
   mkdir -p src/assets/classes && mv public/images/classes/hero-*.webp src/assets/classes/
   ```

   (워터마크 제거 → 위로 캔버스 확장 2.2:1 → 왼쪽 녹이기 → webp 저장. `RATIO`·`FADE`·`WIDTH` 는 손대지 말 것.)

   ★ 최종 자리는 `public/` 이 아니라 **`src/assets/classes/`** 다. `public/` 은 URL 이 고정이라
   ...
   지금 쓰는 `src/assets/hero/class-note.webp` 도 같은 이유로 거기 있다 — **새 두 장이 붙으면 지운다.**
...
3:1 원본을 그대로 깔면 모바일에서 장면이 폭 134px 로 확대돼 안내 문구를 덮는다.
좌우로 밀어도 소용없다 — `cover` 는 높이로 맞추므로 **가로 여백을 늘려도 화면 위 크기가 그대로**다.
그래서 후처리에서 **위쪽에 빈 하늘을 덧대 2.2:1 로 늘린다**(장면 폭 134px → **103px**). 이게 유일한 레버다.

보이는 영역:

| | `cover` 기준 | 보이는 영역 |
|---|---|---|
| 모바일 | 높이맞춤 | 오른쪽 **87%** (세로는 전부, 위 27%는 덧댄 빈 하늘) |
| PC | 폭맞춤 | 가로 전체, **위 51% 잘림** |

→ PC에서 보이는 건 최종 에셋의 **아래 49%** 뿐이다. 덧댄 하늘 27% 를 빼면
**원본(3:1) 기준으로는 아래 66%** 가 보인다. 그래서 프롬프트의 규칙은 이것 하나로 요약된다:

> **원본에서 주인공 머리가 아래에서 62% 를 넘지 않으면 세 경우 다 온전히 보인다.**
> (묵상방 히어로보다 카드가 더 납작해서 위가 더 잘린다 — 이 62% 규칙을 더 엄하게 지킬 것.)

### 글자가 지나가는 자리 (제일 중요)

텍스트는 전부 **왼쪽 정렬**, 안내 문구는 `max-w-[17rem]`(272px)로 묶여 있다.
즉 **모바일에서 카드 왼쪽 296px 위로 글씨가 지나간다** — 카드 폭의 **83%**.
...
| 0.20 (글씨 시작) | 24 | 24 |
| 0.50 (녹이기 시작) | 135 | 416 |
| 0.75 (주인공 왼쪽 끝) | 247 | 624 |
| **0.85 (안내 문구 끝)** | **296** | 707 |
| 0.90 (원래 밝기 회복) | 312 | 749 |
...
`docs/plan-hero-check.py` 를 그대로 쓰되 아래 세 줄만 바꾼다(사각형은 근사치 — 본문은 세로 가운데 정렬이라
문구 줄 수에 따라 ±12px 움직인다. 넉넉하게 잡아 뒀다):

```python
def sample(name, card_w, card_h=187): ...   # 'plans' → 'classes'
RECTS={'라벨':(24,40,120,58),'제목':(24,66,250,110),'안내문':(24,114,296,172)}
...
**주인공이 너무 커서 위가 잘릴 때** (PC에서 위 51%가 잘린다)
...
- **2026-09-12**: 프롬프트만 작성. 에셋 미생성, `ClassList.tsx` 미변경(아직 파란 카드 + 워시 두 겹).
