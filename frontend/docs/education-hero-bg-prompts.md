...
- 두 장 모두 `public/images/education/` 적용 완료 (라이트 26KB · 다크 19KB, 1456×720 원본 유지).
  Gemini 워터마크(✦, 전역 x1312~1360 · y576~624)는 **인페인트가 아니라 배경 되메우기**로 제거 —
  바로 위에 종이비행기가 붙어 있어 TELEA inpaint 는 흰 뭉개짐을 남긴다.
  워터마크 연결성분만 마스크로 뽑고(다크에서 추출, 두 장 위치 동일), 가로로 평평한
  깨끗한 구간(x1294~1308)의 **행별 중앙값**으로 채운 뒤 경계만 깃털링.
- 레이아웃(`education.css` 끝 두 블록):
  - **모바일**: 카드는 `.edu-hero` 가 아니라 **`.edu-hero-main`**. 일러스트는 카드 **폭 150%**·
    `center bottom` 이라 제목 줄 수와 무관하게 교실 띠 높이가 일정(≈120px), 카드 `padding-bottom: 8rem`
    이 그 자리. 위쪽 하늘은 카드색(`--edu-card`) 세로 페이드로 덮어 글자 자리를 만든다.
  - **'한눈에 보기' 스탯은 카드 밖으로** 내보내 회색 캔버스 위 흰 카드 2장이 됐다 —
    안에 두면 교실 위에 상자가 겹친다.
  - **PC**: 띠(약 1200×270)에 `cover; center bottom` → 원본 아래 45%(교실)가 가로 전체에.
    왼쪽 글 자리만 캔버스색(`--edu-band`) 가로 워시(56%에서 소멸)로 누르고, 가운데 칠판·양들은
    워시 밖이라 선명하다. 아래쪽엔 캔버스색 페이드로 잘린 느낌을 없앤다.
- 히어로의 **블루 blur 원(`.edu-hero::before`)은 삭제** — 일러스트 위에서 뿌옇게 뜬다.
- 이미지를 다시 뽑으면 위 두 블록의 워시 정지점만 다시 맞추면 된다.
