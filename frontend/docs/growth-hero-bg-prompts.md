...
큰 숫자 자리가 195~198 로 나왔다(현재 사진은 162/76). 그림 쪽을 어둡게 다시 뽑는 대신
**라이트만 스크림을 한 겹 더 얹어서** 해결했다(`GrowthHero.css` 의 좌측 겹). 화면 실측은
라벨 98 · 숫자 72 · 헤드 41 로, 지금 사진(107/43/34)과 같은 급이 된다.
다시 뽑을 때도 같은 선택지가 있다 — **그림을 어둡게 하거나, 좌측 스크림을 조절하거나.**
...
2. **스크림은 테마별로 겹수가 다르다** (위 [밝기 합격선](#밝기-합격선--두-테마-모두-상한이다) 참고).
   - 라이트: 대각(`.65 → .35 → .10`) + **좌측 한 겹**(`.45 → .18 45% → 0 72%`).
...
- **화면 실측**(스크림까지 얹은 값, 흰 글씨 기준 — 낮을수록 좋다):

  | | 라벨 | 숫자 | 헤드 | 보조 |
  |---|---|---|---|---|
  | 라이트 모바일 416 | 95~98 (p95 120~123) | 71~72 | 40~42 | 30~33 |
  | 라이트 PC 832 | 61~78 | 36~50 | 45 | 30~34 |
  | 다크 모바일 416 | 27~31 | 39~44 | 25~32 | 15~22 |
  | 다크 PC 832 | 23~26 | 21~28 | 26~33 | 15~22 |

  지금 사진의 같은 자리가 라벨 162 · 숫자 76 · 헤드 62 · 보조 73 이었으니, 여섯 장 모두
  **사진보다 대비가 좋다**(라벨은 좌측 스크림 덕에 162 → 98).
