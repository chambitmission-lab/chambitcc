...
1. **워터마크 ✦ 제거.** 우하단에 뜬다. 프롬프트대로 그 자리를 빈 바닥으로 그리게 했으므로
   주변이 평평하다 → 인페인트 말고 **좌우 깨끗한 기둥의 행별 중앙값 사이 선형보간**으로 되메운다
   (`docs/plan-hero-process.py` 의 방식과 동일. TELEA 인페인트는 실루엣을 빨아들여 얼룩이 남는다).
2. **2:1 로 크롭.** `magick in.png -resize 1024x512^ -gravity east -extent 1024x512 out.png`
   — **오른쪽 기준**(`east`)이다. 주인공이 오른쪽에 있으므로 왼쪽을 버린다.
3. **왼쪽 알파 페이드를 굽는다.** x 0~46% 알파 0, 46~78% smoothstep, 78%~ 알파 1.
   위·아래도 각각 6% 씩 살짝 페이드해서 잘린 단면이 안 보이게 한다.
4. 다크는 **전체 알파 ×0.9** 로 한 단계 눌러 앉힌다(흰 글씨가 그림 근처를 지나간다).
5. `cwebp -q 80 -alpha_q 92 -m 6` 로 저장. 35KB 넘으면 `-q 72` 까지 내린다.

## 적용할 때 (아직 미적용)

`TimeCapsuleCard.css` 에 레이어 한 장을 추가하고, **기존 봉투 SVG(`.tc-card__visual`)는 숨긴다.**
봉투가 두 개(그림 속 봉투 + SVG 봉투)가 되면 오른쪽이 뭉갠다.

```css
/* 별·은하수 레이어 위, .tc-card__body 아래 */
.tc-card__art {
  position: absolute;
  inset: 0;
  pointer-events: none;
  background-image: url('/images/capsule/home-banner-dark.webp');
  background-repeat: no-repeat;
  background-position: right center;
  background-size: auto 100%;
}

html:not(.dark) .tc-card__art {
  background-image: url('/images/capsule/home-banner-light.webp');
}
```

- `.tc-card__body` 는 이미 `z-index: 1` 이라 글씨는 그림 위로 올라온다.
- **은하수·별·별똥별 레이어는 그대로 둔다.** 그림이 불투명한 오른쪽에선 어차피 가려지고,
  알파 페이드 구간(카드 가운데)에서 이음매를 메워 준다.
- 참고 시안의 **말풍선**을 살리고 싶으면 `.tc-card__art` 위에 `<span>` 으로 얹는다.
  그림에 그려 넣게 하면 안 된다(한글이 깨진다). 대사는 로테이션 배열로 두는 게 좋다.
- 참고 시안의 **원형 화살표 버튼**을 넣는다면 오른쪽 아래가 아니라 **오른쪽 세로 중앙**,
  그림의 빈 6% 자리다. 넣을 경우 프롬프트의 "RIGHT 6% empty" 를 **12%** 로 올려서 다시 뽑는다.
- **배경 이미지 늦은 발견 함정**(`css-background-image-late-discovery`): CSS 배경은 preload 스캐너에
  안 잡히고 엘리먼트가 그려진 뒤에야 요청된다. 홈 첫 화면에 들어오는 카드이므로
  현재 테마 파일은 `<link rel="preload" as="image">`, **반대 테마는 유휴 시간에 미리 데운다.**
- 적용 후 **모바일에서 기본 카피(캡슐 0개) 상태**를 반드시 확인한다. 상태 문구가 2줄이라
  카드가 152px 로 자라고, 그만큼 그림도 커져서 글자와 겹치는 유일한 케이스다.
  겹치면 그림을 다시 뽑지 말고 `background-size` 를 `auto 88%` 로 낮춘다.
